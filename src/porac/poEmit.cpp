...
void poCodeGenerator::emitArgs(poNode* node, poFlowGraph& cfg)
{
    ...
    for (poNode* child : list->list())
    {
        if (child->type() == poNodeType::PARAMETER)
        {
            ...
            bb->addInstruction(poInstruction(getOrAddVariable(name, code), code, -1, -1, IR_PARAM));
        }
    }
}
...
void poCodeGenerator::emitIfStatement(poNode* node, poFlowGraph& cfg, poBasicBlock* endBB)
{
    ...
    if (elseNode)
    {
        ...
        cfg.addBasicBlock(bodyBB);
        emitBody(body, cfg);
        cfg.getLast()->addInstruction(poInstruction(_instructionCount++, TYPE_I64, IR_JUMP_UNCONDITIONAL, -1, IR_BR)); /*unconditional branch*/
...
void poCodeGenerator::emitIfExpression(poConditionGraphNode& cgn, poFlowGraph& cfg, poBasicBlock* successBB, poBasicBlock* failBB)
{
    ...
    while (id != -1)
    {
        ...
        {
            id = fail.id();
            cfg.getLast()->addInstruction(poInstruction(_instructionCount++, TYPE_I64, getCompare(node.node()), -1, IR_BR));
        ...
        {
            id = success.id();
            cfg.getLast()->addInstruction(poInstruction(_instructionCount++, TYPE_I64, flipJump(getCompare(node.node())), -1, IR_BR));
...
void poCodeGenerator::emitCall(poNode* node, poFlowGraph& cfg)
{
    std::vector<int> args;
    ...
    cfg.getLast()->addInstruction(poInstruction(_instructionCount++, 0, int(call->list().size()), -1, IR_CALL));
    for (int arg : args)
    {
        cfg.getLast()->addInstruction(poInstruction(_instructionCount++, 0, arg, -1, IR_ARG));
    }
}
...
void poCodeGenerator::emitReturn(poNode* node, poFlowGraph& cfg)
{
    ...
    if (returnNode->child())
    {
        ...
        cfg.getLast()->addInstruction(poInstruction(_instructionCount++, 0, expr, -1, IR_RETURN));
    }
    else
    {
...
int poCodeGenerator::emitBinaryExpr(poNode* node, poFlowGraph& cfg)
{
    ...
    case poNodeType::ADD:
        instructionId = _instructionCount;
        cfg.getLast()->addInstruction(poInstruction(_instructionCount++, TYPE_I64, left, right, IR_ADD));
        break;
    case poNodeType::SUB:
        instructionId = _instructionCount;
        cfg.getLast()->addInstruction(poInstruction(_instructionCount++, TYPE_I64, left, right, IR_SUB));
        break;
    case poNodeType::MUL:
        instructionId = _instructionCount;
        cfg.getLast()->addInstruction(poInstruction(_instructionCount++, TYPE_I64, left, right, IR_MUL));
        break;
    case poNodeType::DIV:
        instructionId = _instructionCount;
        cfg.getLast()->addInstruction(poInstruction(_instructionCount++, TYPE_I64, left, right, IR_DIV));
        break;
    case poNodeType::CMP_EQUALS:
    case poNodeType::CMP_NOT_EQUALS:
    case poNodeType::CMP_LESS:
    ...
    case poNodeType::CMP_GREATER_EQUALS:
        instructionId = _instructionCount;
        cfg.getLast()->addInstruction(poInstruction(_instructionCount++, TYPE_I64, left, right, IR_CMP));
        break;
    }
    return instructionId;
}
