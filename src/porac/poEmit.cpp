...
void poEmitter::setDebugInfo(const poDebugInfo& info)
{
    _info = info;
}

void poEmitter::setDebugInfo(const int fileId, const int line, const int column)
{
    _info = poDebugInfo(fileId, line, column);
}
...
int poEmitter::emitConstant(const int64_t i64, poFlowGraph& cfg)
{
    ...
    emitInstruction(poInstruction(_instructionCount++, TYPE_I64, id, IR_CONSTANT, _info), cfg.getLast());
...
int poEmitter::emitConstant(const uint64_t u64, poFlowGraph& cfg)
{
    ...
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, TYPE_U64, id, IR_CONSTANT, _info), cfg.getLast());
    return instructionId;
}
...
int poEmitter::emitConstant(const int32_t i32, poFlowGraph& cfg)
{
    ...
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, TYPE_I32, id, IR_CONSTANT, _info), cfg.getLast());
    return instructionId;
}
...
int poEmitter::emitConstant(const uint32_t u32, poFlowGraph& cfg)
{
    ...
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, TYPE_U32, id, IR_CONSTANT, _info), cfg.getLast());
    return instructionId;
}
...
int poEmitter::emitConstant(const int16_t i16, poFlowGraph& cfg)
{
    ...
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, TYPE_I16, id, IR_CONSTANT, _info), cfg.getLast());
    return instructionId;
}
...
int poEmitter::emitConstant(const uint16_t u16, poFlowGraph& cfg)
{
    ...
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, TYPE_U16, id, IR_CONSTANT, _info), cfg.getLast());
    return instructionId;
}
...
int poEmitter::emitConstant(const int8_t i8, poFlowGraph& cfg)
{
    ...
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, TYPE_I8, id, IR_CONSTANT, _info), cfg.getLast());
    return instructionId;
}
...
int poEmitter::emitConstant(const uint8_t u8, poFlowGraph& cfg)
{
    ...
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, TYPE_U8, id, IR_CONSTANT, _info), cfg.getLast());
    return instructionId;
}
...
int poEmitter::emitConstant(const float f32, poFlowGraph& cfg)
{
    ...
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, TYPE_F32, id, IR_CONSTANT, _info), cfg.getLast());
    return instructionId;
}
...
int poEmitter::emitConstant(const double f64, poFlowGraph& cfg)
{
    ...
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, TYPE_F64, id, IR_CONSTANT, _info), cfg.getLast());
    return instructionId;
}
...
int poEmitter::emitConstant(const std::string& str, poFlowGraph& cfg)
{
    ...
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, poUtil::getPointerType(_module, TYPE_U8), id, IR_CONSTANT, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, type, constantId, IR_CONSTANT, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(left), int16_t(right), IR_ADD, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(left), int16_t(right), IR_SUB, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(left), int16_t(right), IR_MUL, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(left), int16_t(right), IR_DIV, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(left), int16_t(right), IR_LEFT_SHIFT, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(left), int16_t(right), IR_RIGHT_SHIFT, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(left), int16_t(right), IR_MODULO, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(left), int16_t(right), IR_AND, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(left), int16_t(right), IR_OR, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(left), -1, IR_UNARY_MINUS, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(left), int16_t(right), IR_CMP, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(returnType), int16_t(numArgs), int16_t(symbolId), IR_CALL, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(arg), -1, IR_ARG, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(type), int16_t(value), -1, IR_RETURN, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(TYPE_VOID), -1, -1, IR_RETURN, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(dstType), int16_t(value), -1, int16_t(srcType), IR_SIGN_EXTEND, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(dstType), int16_t(value), -1, int16_t(srcType), IR_ZERO_EXTEND, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(dstType), int16_t(value), -1, int16_t(srcType), IR_BITWISE_CAST, _info), cfg.getLast());
    return instructionId;
}
...
{
    const int instructionId = _instructionCount;
    emitInstruction(poInstruction(_instructionCount++, int16_t(dstType), int16_t(value), -1, int16_t(srcType), IR_CONVERT, _info), cfg.getLast());
    return instructionId;
}
...
int poEmitter::emitAlloca(const int type, poBasicBlock* bb)
{
    ...
    const int ptrType = poUtil::getPointerType(_module, type);
    emitInstruction(poInstruction(_instructionCount++, ptrType, 1 /* num elements */, -1, IR_ALLOCA, _info), bb);
    return id;
}
...
{
    const int ptrType = poUtil::getPointerType(_module, type);
    bb->addInstruction(poInstruction(varName, ptrType, 1 /* num elements */, -1, IR_ALLOCA, _info));
    return varName;
}
...
{
    const int id = _instructionCount++;
    emitInstruction(poInstruction(id, type, value, -1, offset, IR_PTR, _info), cfg.getLast());
    return id;
}
...
{
    const int id = _instructionCount++;
    emitInstruction(poInstruction(id, type, value, dynamicOffset, offset, IR_PTR, _info), cfg.getLast());
    return id;
}
...
{
    const int id = _instructionCount++;
    emitInstruction(poInstruction(id, type, value, element, IR_ELEMENT_PTR, _info), cfg.getLast());
    return id;
}
...
{
    const int id = _instructionCount++;
    emitInstruction(poInstruction(id, type, ptr, -1, IR_LOAD, _info), cfg.getLast());
    return id;
}
...
{
    const int id = _instructionCount++;
    emitInstruction(poInstruction(id, type, ptr, value, IR_STORE, _info), cfg.getLast());
    return id;
}
...
{
    const int id = _instructionCount++;
    emitInstruction(poInstruction(id, 0, branchType, -1, IR_BR, _info), cfg.getLast());
    return id;
}
...
{
    const int id = _instructionCount++;
    emitInstruction(poInstruction(id, type, branchType, -1, IR_BR, _info), cfg.getLast());
    return id;
}
...
{
    const int id = _instructionCount++;
    emitInstruction(poInstruction(id, type, paramIndex, -1, IR_PARAM, _info), cfg.getLast());
    return id;
}
...
int poEmitter::emitStoreGlobal(const int type, const int value, const int globalId, poFlowGraph& cfg)
{
    ...
    emitInstruction(poInstruction(store, type, -1, value, globalId, IR_STORE_GLOBAL, _info), cfg.getLast());
    return store;
}
...
int poEmitter::emitLoadGlobal(const int type, const int globalId, poFlowGraph& cfg)
{
    ...
    emitInstruction(poInstruction(load, type, -1, -1, globalId, IR_LOAD_GLOBAL, _info), cfg.getLast());
    return load;
}
...
void poCodeGenerator::generate(const std::vector<poNode*>& ast)
{
    ...
    for (auto& ns : _module.namespaces())
    {
        ...
        {
            if (_module.types()[type].isGeneric()) {
                continue;
            }
            ...
            for (int i = 0; i < numConstructors; i++)
            {
                ...
                const std::string fullName = ns.name() + "::" + typeData.name() + "::" + constructor.name();
...
void poCodeGenerator::emitArrayCopy(const int src, const int dst, poFlowGraph& cfg, const int size)
{
    ...
    const int pointerType = poUtil::getPointerType(_module, srcType.id());
...
void poCodeGenerator::emitConstructor(poFlowGraph& cfg)
{
    ...
    // TODO: this may go unused if there is no constructors to call for the fields
    ...
    for (int i = 0; i < int(baseTypeData.fields().size()); i++)
    {
        ...
        if (constructorId != -1)
        {
            ...
            const int memberPtr = poUtil::getPointerType(_module, fieldTypeData.baseType());
    ...
    for (int i = 0; i < int(baseTypeData.fields().size()); i++)
    {
        ...
        if (constructorId != -1)
        {
            const int memberPtr = poUtil::getPointerType(_module, field.type());
...
void poCodeGenerator::emitDefaultConstructor(poFunction& function, const int type)
{
    ...
    const poType& typeData = _module.types()[type];
    const int ptrType = poUtil::getPointerType(_module, poUtil::getPointerType(_module, type));
...
void poCodeGenerator::emitFunction(poNode* node, poFunction& function)
{
    ...
    for (poNode* child : list->list())
    {
        ...
        else if (child->type() == poNodeType::RETURN_TYPE)
        {
            ...
            {
                retType = poUtil::getPointerType(_module, retType);
            }
        }
    ...
    if (type.size() > 8)
    {
        ...
        _returnInstruction = _emitter.addVariable(poUtil::getPointerType(_module, retType), QUALIFIER_NONE);
    }
...
void poCodeGenerator::emitArgs(poNode* node, poFlowGraph& cfg)
{
    ...
    for (poNode* child : list->list())
    {
        if (child->type() == poNodeType::PARAMETER)
        {
            ...
            int code = poUtil::unpackTypeNode(_module, type);

            if (code == -1) {
                code = getType(type->token());
            }

            const poType& baseType = _module.types()[code];

            const int pointerType = poUtil::getPointerType(_module, baseType.id());
...
int poCodeGenerator::emitNewArray(poNode* node, poFlowGraph& cfg)
{
    ...
    const int baseType = poUtil::unpackTypeNode(_module, arrayNode->right());
    // getType(arrayNode->token());
    ...
    const int numElements = emitExpr(arrayNode->left(), cfg);

    const int ptrType = poUtil::getPointerType(_module, baseType);
    ...
    {
        _emitter.setDebugInfo(node->token().fileId(),
                    node->token().line(),
                    node->token().column());
...
int poCodeGenerator::emitNew(poNode* node, poFlowGraph& cfg)
{
    ...
    const int type = getType(constructor->token());
     if (type == -1)
    {
        setError("Undefined type '" + constructor->token().string() + "' for 'new' operator.", node->token());
        return EMIT_ERROR;
    }
...
int poCodeGenerator::emitCall(poNode* node, poFlowGraph& cfg, const int instanceExpr)
{
    ...
    for (poNode* child : functionNode->list())
    {
        ...
        else if (child->type() == poNodeType::RETURN_TYPE)
        {
            ...
            if (returnTypeNode->child()->type() == poNodeType::POINTER)
            {
                ...
                returnType = poUtil::getPointerType(_module, returnType);
            }
        }
    }
    ...
    for (int i = 0; i < int(argsNode->list().size()); i++)
    {
        ...
        int type = poUtil::unpackTypeNode(_module, param->child());
        if (type == -1) {
            type = getType(param->child()->token());
        }
    ...
    if (typeData.baseType() == TYPE_OBJECT)
    {
        if (typeData.size() > 8)
        {
            ...
            const int returnPtr = poUtil::getPointerType(_module, returnType);
    ...
    for (int i = 0; i < int(args.size()); i++)
    {
        ...
        if (typeData.baseType() == TYPE_OBJECT)
        {
            if (typeData.size() > 8)
            {
                ...
                const int pointerType = poUtil::getPointerType(_module, type);
    ...
    if (typeData.baseType() == TYPE_OBJECT && typeData.size() <= 8)
    {
        ...
        const int pointerType = poUtil::getPointerType(_module, typeData.id());
...
void poCodeGenerator::emitLoopPreHeader(poFlowGraph& cfg, const int instanceExpr)
{
    ...
    _pointerType = poUtil::getPointerType(_module, typeData.baseType());
...
{
    poDebugInfo info(node->token().fileId(),
            node->token().line(),
            node->token().column());

    _emitter.setDebugInfo(info);
...
void poCodeGenerator::emitAssignment(poNode* node, poFlowGraph& cfg)
{
            ...
            if (_thisInstruction != -1)
            {
                ...
                if (thisTypeData.isPointer())
                {
                    ...
                    if (fieldType != -1)
                    {
                        ...
                        const int pointerType = poUtil::getPointerType(_module, fieldType);
    ...
    {
        poBinaryNode* dynamicArrayNode = static_cast<poBinaryNode*>(assignment->left());
        assert(dynamicArrayNode->type() == poNodeType::DYNAMIC_ARRAY);

        poNode* variableNode = dynamicArrayNode->left();
        assert(variableNode->type() == poNodeType::VARIABLE);

        const int variable = getVariable(variableNode->token().string());
        const int varType = _emitter.getType(variable);
        const int pointerType = poUtil::getPointerType(_module, varType);

        const int ptr = _emitter.emitPtr(pointerType, variable, 0, cfg);
        _emitter.emitStore(pointerType, ptr, right, cfg);
    }
    else
    {
...
void poCodeGenerator::emitDecl(poNode* node, poFlowGraph& cfg)
{
    ...
    if (declNode->child()->type() == poNodeType::ASSIGNMENT)
    {
        ...
        if (assignment->left()->type() == poNodeType::POINTER)
        {
            ...
            const int baseType = poUtil::getPointerType(_module, type);
            const int ptrType = poUtil::getPointerType(_module, baseType);
        ...
        else if (assignment->left()->type() == poNodeType::DYNAMIC_ARRAY)
        {
            ...
            poNode* typeNode = dynamicArrayNode->right();
            int baseType = poUtil::unpackTypeNode(_module, typeNode);
            if (baseType == -1) {
                baseType = getType(typeNode->token());
                if (baseType == -1) {
                    setError("Type mismatch for dynamic array.", typeNode->token());
                }
            }

            const int pointerType = poUtil::getPointerType(_module, baseType);
            const int pointerType2 = poUtil::getPointerType(_module, pointerType);
    ...
    else if (declNode->child()->type() == poNodeType::POINTER)
    {
        ...
        const int pointerType = poUtil::getPointerType(_module, type); /* tODO: we need to get the pointer type with respect to the pointer count.. */
        const int storeType = poUtil::getPointerType(_module, pointerType);
    ...
    else if (declNode->child()->type() == poNodeType::ARRAY)
    {
        ...
        const int arrayType = getArrayType(poUtil::getPointerType(_module, type), 1);
        const std::string name = variable->token().string();
    ...
    else if (declNode->child()->type() == poNodeType::CONSTRUCTOR)
    {
        ...
        for (poNode* childNode : constructor->list())
        {
            ...
            else if (childNode->type() == poNodeType::GENERIC)
            {
                genericNode = static_cast<poGenericNode*>(childNode);
                variableNode = genericNode->child();
            }
        }
        ...
        int declType = type;
        if (genericNode) {
            std::string typeName = _module.types()[type].name() + "<";
            for (int i = 0; i < int(genericNode->nodes().size()); i++) {
                if (i > 0) {
                    typeName += ",";
                }

                poNode* param = genericNode->nodes()[i];
                int paramType = poUtil::getType(param->token());
                if (paramType == -1) {
                    paramType = poUtil::unpackTypeNode(_module, param);
                }

                if (paramType != -1) {
                    typeName += _module.types()[paramType].name();
                }
            }

            typeName += ">";
            declType = _module.getTypeFromName(typeName);
        }
        ...
        if (variableNode->type() == poNodeType::POINTER)
        {
            ...
            declType = poUtil::getPointerType(_module, type);
        }
        ...
        {
            const int ptrType = poUtil::getPointerType(_module, declType);
...
int poCodeGenerator::emitCast(poNode* node, poFlowGraph& cfg)
{
    ...
    int dstType = poUtil::unpackTypeNode(_module, typeNode);
    if (arrayNode && dstType != EMIT_ERROR) {
        dstType = getArrayType(dstType, 1);
    }

    const int expr = emitExpr(castNode->right(), cfg);
    if (expr == EMIT_ERROR ||
        dstType == EMIT_ERROR)
    {
    ...
    else if (dstTypeData.isArray())
    {
        ...
        {
            const int pointerType = poUtil::getPointerType(_module, dstTypeData.baseType());
...
int poCodeGenerator::emitReference(poNode* node, poFlowGraph& cfg)
{
    ...
    {
        if (_thisInstruction != -1) {
            // This gives the type of a Pointer to the this Pointer.
            const int thisTypePtr = _emitter.getType(_thisInstruction);
            const poType& thisTypePtrData = _module.types()[thisTypePtr];
            // Get the base type
            const poType& thisTypeData = _module.types()[thisTypePtrData.baseType()];
            const int baseType = thisTypeData.baseType();
            const poType& type = _module.types()[baseType];
            for (const poField& field : type.fields()) {
                if (field.name() == child->token().string()) {
                    const int thisId =  emitLoadThis(cfg);
                    expr = _emitter.emitPtr(poUtil::getPointerType(_module, field.type()), thisId, field.offset(), cfg);
                    break;
                }
            }
        }
...
int poCodeGenerator::emitSizeof(poNode* node, poFlowGraph& cfg)
{
    ...
    if (pointerCount > 0)
    {
        type = poUtil::getPointerType(_module, type);
    }
    const poType& typeData = _module.types()[type];
    ...
    if (varName == EMIT_ERROR)
    {
        if (_thisInstruction != EMIT_ERROR)
        {
            ...
            if (thisTypeData.isPointer())
            {
                ...
                {
                    return _emitter.emitPtr(poUtil::getPointerType(_module, fieldType), _thisInstruction, offset, cfg);
                }

                if (fieldTypeData.isArray())
                {
                    const int ptrType = poUtil::getPointerType(_module, fieldTypeData.baseType());
                    const int ptrIns = _emitter.emitPtr(poUtil::getPointerType(_module, ptrType), _thisInstruction, offset, cfg);

                    return _emitter.emitLoad(ptrType, ptrIns, cfg);
                }

                const int thisVar = emitLoadThis(cfg);
                const int ptr = _emitter.emitPtr(poUtil::getPointerType(_module, fieldType), thisVar, offset, cfg);
...
void poCodeGenerator::emitStoreArray(poNode* node, poFlowGraph& cfg, const int id)
{
    ...
    if (type.isArray())
    {
        ...
        const int pointerType = poUtil::getPointerType(_module, type.baseType());
        ...
        if (_module.types()[type.baseType()].baseType() == TYPE_OBJECT) {
            emitCopy(id, ptr, cfg);
        }
        else {
...
void poCodeGenerator::emitStoreMember(poNode* node, poFlowGraph& cfg, const int id)
{
    ...
    for (int i = int(stack.size() - 1); i >= 0; i--)
    {
        ...
        if (member->type() == poNodeType::ARRAY_ACCESSOR)
        {
            ...
            poDebugInfo info(
                    accessor->token().fileId(),
                    accessor->token().line(),
                    accessor->token().column());
    ...
    const int storePtr = poUtil::getPointerType(_module, fieldType);
...
int poCodeGenerator::emitLoadMember(poNode* node, poFlowGraph& cfg)
{
    ...
    if (dynamicOffsetIns == -1)
    {
        const int loadPtr = poUtil::getPointerType(_module, fieldType);
...
int poCodeGenerator::emitConstantExpr(poNode* node, poFlowGraph& cfg)
{
    ...
    poConstantPool& constants = _module.constants();
    poDebugInfo info(
            constant->token().fileId(),
            constant->token().line(),
            constant->token().column());
    int instructionId = EMIT_ERROR;
